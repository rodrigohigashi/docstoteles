...
[Nightscout](http://www.nightscout.info/)
 (CGM in the Cloud) is an open source, DIY project that allows real time access to a CGM data via an HTTP REST API. It is used for secure remote viewing of blood sugar data from anywhere...including Oh My Posh segments on the command line!
...
*   toml
    ...
    {  "type": "nightscout",  "style": "diamond",  "foreground": "#ffffff",  "background": "#ff0000",  "background_templates": [    "{{ if gt .Sgv 150 }}#FFFF00{{ end }}",    "{{ if lt .Sgv 60 }}#FF0000{{ end }}",    "#00FF00"  ],  "foreground_templates": [    "{{ if gt .Sgv 150 }}#000000{{ end }}",    "{{ if lt .Sgv 60 }}#000000{{ end }}",    "#000000"  ],  "leading_diamond": "",  "trailing_diamond": "",  "template": " {{ .Sgv }}{{ .TrendIcon }}",  "options": {    "url": "https://YOURNIGHTSCOUTAPP.herokuapp.com/api/v1/entries.json?count=1&token=APITOKENFROMYOURADMIN",    "http_timeout": 1500  }}

    type: nightscoutstyle: diamondforeground: "#ffffff"background: "#ff0000"background_templates:  - "{{ if gt .Sgv 150 }}#FFFF00{{ end }}"  - "{{ if lt .Sgv 60 }}#FF0000{{ end }}"  - "#00FF00"foreground_templates:  - "{{ if gt .Sgv 150 }}#000000{{ end }}"  - "{{ if lt .Sgv 60 }}#000000{{ end }}"  - "#000000"leading_diamond: trailing_diamond: template:  {{ .Sgv }}{{ .TrendIcon }}options:  url: https://YOURNIGHTSCOUTAPP.herokuapp.com/api/v1/entries.json?count=1&token=APITOKENFROMYOURADMIN  http_timeout: 1500
...
Options[​](https://ohmyposh.dev/docs/segments/health/nightscout#options "Direct link to Options")

--------------------------------------------------------------------------------------------------

| Name | Type | Default | Description |
| --- | --- | --- | --- |
| `url` | [`template`](https://ohmyposh.dev/docs/configuration/templates) |     | Your Nightscout URL, including the full path to entries.json AND count=1 AND token. Example above. You'll know this works if you can curl it yourself and get a single value |
...
You can change the icons for trend, put the trend elsewhere, add text, however you like! Make sure your NerdFont has the glyph you want or [search for one](https://www.nerdfonts.com/cheat-sheet)
.
...
)[​](https://ohmyposh.dev/docs/segments/health/nightscout#template-info "Direct link to template-info")
...
*   [Options](https://ohmyposh.dev/docs/segments/health/nightscout#options)
